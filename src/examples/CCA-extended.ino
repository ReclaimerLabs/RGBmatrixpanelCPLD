/**
 * @file cca-extended.ino
 * @author Alex Hiam - <alex@graycat.io>
 *
 * @brief An extended version of the A Cyclic Cellular Automaton demo for 
 *        the Reclaimer Labs Internet Window.
 * 
 * CCA engine and example for the Internet Window.
 * Extended from CyclicCellularAutomata.ino to Displays a number of 
 * different CCA rule sets and color schemes, either sequentially or
 * randomly.
 * 
...
#define RANDOM      1   ///< Set to 1 to have configurations selected randomly, 0 for sequential
...
// Create a sequence of CCA configurations:
static CCA_Config sg_configurations[] = {
    ...
    { // Amoeba from http://psoup.math.wisc.edu/mcell/rullex_cycl.html
        3,               // Range = 3 cells
        10,              // Threshold = 10 cells
        2,               // 2 possible states
        NEIGHBORHOOD_VN, // Von Neumann neighborhood

        0,               // State 0 = min_hue
        HUE_RED,         // Min hue = 0
        HUE_AQUA,        // Max hue = 1535 (full range)
        128,             // Saturation = 50%
        77,              // Brightness = 30%
    ...
    { // Cubism from http://psoup.math.wisc.edu/mcell/rullex_cycl.html
        3,               // Range = 3 cells
        5,               // Threshold = 5 cells
        3,               // 3 possible states
        NEIGHBORHOOD_VN, // Von Neumann neighborhood
    ...
    { // Black vs White from http://psoup.math.wisc.edu/mcell/rullex_cycl.html
        5,               // Range = 5 cells
        23,              // Threshold = 23 cells
        2,               // 2 possible states
        NEIGHBORHOOD_VN, // Von Neumann neighborhood
    ...
    { // Different color Cubism, with black
        3,               // Range = 3 cells
        5,               // Threshold = 5 cells
        3,               // 3 possible states
        NEIGHBORHOOD_VN, // Von Neumann neighborhood
    ...
    { // Idk, it looks kinda cool
        ...
        0,               // Wrap edges
        0,               // No delay between steps
        60000            // Run for 2 minutes
    },
};
#define N_RULES (sizeof(sg_configurations) / sizeof(CCA_Config))


// Internet Window display instance:
static RGBmatrixPanelCPLD sg_display(WIDTH, HEIGHT);

// Instantiate a neighborhood:
...
/**
 * Get the next configuration to use.
 * 
 * If #RANDOM is defined as non-zero, the next configuration will be
 * selected randomly, otherwise it will be the next config in the array.
 * Any config with a duration set to 0 will be skipped.
 * 
 * @return Returns a pointer to the next #CCA_Config configuration
 */
CCA_Config *get_next_config(void);



CCA_Config *get_next_config(void) {
    static uint8_t config_num = 255;

    do {
        if (RANDOM) {
            // Randomly select:
            config_num = (uint8_t) random(0, N_RULES);
        }
        else {
    ...
    } while(sg_configurations[config_num].duration_ms == 0);
    // That loop won't exit until a config with a non-zero duration has 
    // been selected.
    
    return &sg_configurations[config_num];
}
...
void loop() {
    ...
    if (epoch == 0 || millis() - epoch >= (sg_neighborhood.config.duration_ms)) {
        // It's the dawn of time, get the next configuration:
        config = get_next_config();
        // We now have a pointer to the next config, need to dereference to pass
        // a copy to configure_neighborhood:
        configure_neighborhood(&sg_neighborhood, (*config));
    ...
    if (sg_neighborhood.config.step_period > elapsed_time) {
        delay(sg_neighborhood.config.step_period - elapsed_time);
    }
}
